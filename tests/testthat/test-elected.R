...
test_that("the cache directory resolves argument, env var, option, then tempdir", {
  withr::local_envvar(ELECTEDBR_CACHE_DIR = NA)
  withr::local_options(electedBR.cache_dir = NULL)
  d <- elected_cache_dir()
  expect_true(dir.exists(d))
  expect_true(startsWith(d, normalizePath(tempdir(), mustWork = FALSE)))
  opt <- file.path(tempdir(), "electedBR-opt")
  withr::local_options(electedBR.cache_dir = opt)
  expect_identical(elected_cache_dir(), normalizePath(opt))
  env <- file.path(tempdir(), "electedBR-env")
  withr::local_envvar(ELECTEDBR_CACHE_DIR = env)
  expect_identical(elected_cache_dir(), normalizePath(env))
  arg <- file.path(tempdir(), "electedBR-arg")
  expect_identical(elected_cache_dir(arg), normalizePath(arg))
  expect_identical(diretorio_cache_eleitos(arg), elected_cache_dir(arg))
  expect_error(elected_cache_dir(""), "non-empty")
})
